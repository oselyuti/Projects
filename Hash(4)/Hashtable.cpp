...
void hash_table::showload(){
	cout<<right<<"** N = "<<setw(6)<<table.size()<<" load = "<<setprecision(2)<<double(num_inuse)/(double)table.size()<<'\n';
}
...
int main(int argc, char *argv[]) {
	if (argc < 3){//because I need a filename (not optional) write up says check if less than 1
	...
	hash_table H(N);// i got the last 9 wrong according to the gradescript. tried to figure it out..
					//but whatever I compared with the solution executable worked, so...
					//i think it has to do with decimal places maybe?
	if (show) {
		H.set_showstats();
		}
